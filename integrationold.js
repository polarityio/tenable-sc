...
function getAuthToken(options, callback) {
  ...
  request(
    ...
    (err, resp, body) => {
      ...
      tokenCache.set(cacheKey, body.response.token);

      Logger.trace({ tokenCache: tokenCache }, "Checking TokenCache");

      callback(null, body.response.token);
    }
  );
}
...
function doLookup(entities, options, cb) {
  ...
  getAuthToken(options, (err, token) => {
    ...
    entities.forEach(entity => {
      ...
      let requestOptions = {
        method: "GET",
        uri: `${options.url}/rest/deviceInfo?ip=${entity.value}`,
        headers: {
      ...
      tasks.push(function(done) {
        requestWithDefaults(requestOptions, function(error, res, body) {
          ...
          if (res.statusCode === 200) {
            ...
            return done({
              err: body,
              detail: `${body.error_code}: ${body.error_msg}`
            });
          }
